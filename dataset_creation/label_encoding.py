...
def transform_labels(labels, data_generation_config):
    ...
    if label_type == 'isup':
        ...
        raise NotImplementedError('only label_types "isup" are supported')
...
def label_to_int(label_batch, label_type):
    ...
    if label_type == 'isup':
        ...
        raise NotImplementedError('Right now, only label_encodings "isup" available')
...
def int_to_string_label(int_label, label_type):
    ...
    """
    if label_type == 'isup':
        label_list = ['0', '3+3 / 1', '3+4 / 2', '4+3 / 3', '3+5/4+4/5+3 / 4', '4+5/5+4/5+5 / 5']
