...
from dataset_creation.dataset_processing import tf_read_image, create_patched_dataset, get_class_distribution, squeeze_dataset


AUTOTUNE = tf.data.experimental.AUTOTUNE


def create_dataset(data_generation_config={}, client_config={}, usage_mode='train', _config=None, is_federated=False, client_name=""):
    """
    ...
    :param data_generation_config: a dictionary with
                                                  ...
                                                  image_directory: directory where the images are stored (/data)
...
def create_test_dataset(data_generation_config={}, client_config={}, filename="", usage_mode='test', _config=None, is_federated=False, client_name="", test_id=-1):
    """
    read images and labels from .csv file, where the image paths are stored in column 'img_path'
    and the labels are stored in data_generation_config['annotation_column']
...
def dataset_from_csv(data_generation_config, csv_filename, image_directory, batch_size=0, usage_mode='train', _log=logging, _config=None, is_federated=False, client_name="", test_id=-1):
    """
    ...
    if data_generation_config['cache'] is not None:
        if test_id >= 0:
            cache_string = f"{usage_mode}_{test_id}"
        else:
            cache_string = usage_mode
        if is_federated:
            dataset = dataset.cache(data_generation_config['cache'] + cache_string)
        else:
            dataset = dataset.cache(data_generation_config['cache'] + cache_string + "_" + client_name)
    else:
    ...
    if usage_mode == 'train':
        dataset = dataset.shuffle(tf.cast(tf.reduce_min((100, tf.reduce_sum(class_distribution))), 'int64'), reshuffle_each_iteration=True)
